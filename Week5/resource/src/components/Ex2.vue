...
const leftToWin = computed (() =>{
    let currentMax = 0
    if (scoreA.value > scoreB.value){
        currentMax = scoreA.value
    } else {
        currentMax = scoreB.value
    }

    return maxScore.value - currentMax
})
...
<template>
    ...
    <div style="font-family: Arial; max-width: 520px; margin: 24px auto;">
        ...
        <div>
            ...
            <p>Points Left to Win: {{leftToWin}} </p>
        </div>
        ...
        <div style="margin-top: 14px;">
            <!-- C. Display winner / status here -->
             ...
             <p v-if="scoreA == 10"> Winner: {{ teamA }}</p>
